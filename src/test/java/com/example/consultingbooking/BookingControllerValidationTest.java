...
class BookingControllerValidationTest {
    ...
    @Test
    void rejectReasonShorterThanMinimumReturnsValidationError() throws Exception {
        Mockito.when(authService.requireUser("token")).thenReturn(new UserAccount());
    ...
    @Test
    void quoteWithMissingSpecialistIdReturnsValidationError() throws Exception {
        mockMvc.perform(post("/api/bookings/quote")
                        ...
                        .content("""
                                ...
                                """))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.fieldErrors.specialistId").exists());
    }
    ...
    @Test
    void feeBreakdownDownloadReturnsPdfDocument() throws Exception {
        Mockito.when(authService.requireUser("token")).thenReturn(new UserAccount());
        Mockito.when(bookingService.feeBreakdownPdf(Mockito.any(), Mockito.eq(4L)))
                .thenReturn("%PDF-1.4\n%%EOF\n".getBytes(java.nio.charset.StandardCharsets.ISO_8859_1));

        mockMvc.perform(get("/api/bookings/4/fee-breakdown.pdf")
                        .header(AuthService.AUTH_HEADER, "token"))
                .andExpect(status().isOk())
                .andExpect(header().string("Content-Disposition", "attachment; filename=booking-4-fee-breakdown.pdf"))
